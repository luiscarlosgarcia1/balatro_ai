...
def test_unified_scorer_blueprint_copies_right_hand_joker_retrigger_effect():
    score, breakdown = _score_with_jokers([{"name": "Blueprint"}, {"name": "Hanging Chad"}])

    assert score == 60
    assert breakdown["card_retriggers"] == 4


def test_unified_scorer_brainstorm_copies_leftmost_joker_retrigger_effect():
    score, breakdown = _score_with_jokers([{"name": "Hanging Chad"}, {"name": "Brainstorm"}])

    assert score == 60
    assert breakdown["card_retriggers"] == 4


def test_unified_scorer_applies_persistent_madness_xmult_from_unified_game_state():
    state = UnifiedGameState(
        deck=[Card(Rank.ACE, Suit.SPADES)],
        hand_indexes=[0],
        jokers=[JokerInfo(64, "Madness", 7, "xMult grows on blind select")],
    )
    state.madness_xmult = {0: 2.5}
    cards = state.to_dict()["hand"]
    context = ScoringContext(
        cards=cards,
        scoring_cards=cards,
    ...
    assert score == 40
    assert breakdown["joker_x_mult"] == 2.5
    assert "Madness (scoring): +0c +0m x2.5" in breakdown["effects_applied"]
...
def test_tarot_item_creation_is_materialized_once_when_used_in_play(monkeypatch):
    monkeypatch.setattr("balatro_gym.core.consumables.random.choice", lambda sequence: sequence[0])
    ...
    assert state.consumables == ["Mercury", "Mercury"]
