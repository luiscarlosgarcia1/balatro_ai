...
def test_wheel_probability_multiplier_stacks_only_for_active_oops_copies():
    hand_cards = [
        Card(Rank.ACE, Suit.SPADES),
        Card(Rank.KING, Suit.HEARTS),
        Card(Rank.QUEEN, Suit.DIAMONDS),
    ...
    rolls = [0.20, 0.30, 0.10, 0.24, 0.60, 0.80]
    manager.rng = SequenceFloatRng(rolls)
    stacked_effects = manager.on_hand_drawn(
        hand_cards,
        {
            "jokers": [
                {"name": "Oops! All 6s", "disabled": False},
                {"name": "Oops! All 6s", "disabled": False},
                {"name": "Oops! All 6s", "disabled": True},
            ]
        },
    )

    assert stacked_effects["face_down_cards"] == [0, 1, 2, 3]


def test_chicot_disables_boss_on_setting_blind_but_preserves_boss_round_identity():
    chicot = next(j for j in JOKER_LIBRARY if j.name == "Chicot")
    state = UnifiedGameState(ante=2, round=3, jokers=[chicot], hand_size=8)
    game = SimpleNamespace(blinds=[0, 0, 0], blind_index=2, hand_size=8)
    handler = BlindSelectHandler(state, game, BossBlindManager(), None, DeterministicRNG(1))

    state.pending_boss_blind = BossBlindType.THE_MANACLE
    reward, terminated, info = handler.step(Action.SELECT_BLIND_BASE + 2)

    assert terminated is False
    assert reward > 0
    assert state.active_boss_blind == BossBlindType.THE_MANACLE
    assert state.boss_blind_active is False
    assert state.hand_size == 8
    assert game.hand_size == 8
    assert state.chips_needed == 1600
    assert info["boss_disabled"] is True
    assert info["modifications"] == {}


def test_blueprint_does_not_disable_boss_through_chicot_copy_contexts():
    blueprint = next(j for j in JOKER_LIBRARY if j.name == "Blueprint")
    state = UnifiedGameState(ante=2, round=3, jokers=[blueprint], hand_size=8)
    game = SimpleNamespace(blinds=[0, 0, 0], blind_index=2, hand_size=8)
    handler = BlindSelectHandler(state, game, BossBlindManager(), None, DeterministicRNG(1))
    ...
    assert info["boss_disabled"] is False
