...
def test_shop_mask_uses_credit_card_debt_allowance_for_pack_buys_and_rerolls():
    state = UnifiedGameState(
        phase=Phase.SHOP,
        money=0,
        shop_reroll_cost=5,
        ...
        shop_inventory=[
            {"item_type": "PACK", "cost": 4},
            {"item_type": "JOKER", "cost": 21},
        ],
    )
...
def test_contract_shop_mask_kwargs_support_credit_card_debt_allowance():
    mask = build_action_mask(
        phase=Phase.SHOP,
        shop_items=[(4, True), (21, True)],
        money=0,
        credit_card_count=1,
        shop_reroll_cost=5,
        joker_count=0,
        sellable_consumable_count=0,
    )
