...
class CompleteJokerEffects:
    ...
    def apply_joker_effect(
        ...
        if not joker_name or joker_name not in self._SUPPORTED_NAMES:
            return None
        if joker_name in {"Blueprint", "Brainstorm"}:
            return self._copied_joker_effect(joker_name, context, game_state)
    ...
    def get_retrigger_count(
        ...
        self._active_joker_key = self._state_key(joker_name, context)
        if joker_name in {"Blueprint", "Brainstorm"}:
            return self._copied_joker_retrigger_count(joker_name, context, game_state)
    ...
    def _copied_joker_effect(
        self,
        joker_name: str,
        context: Dict[str, Any],
        game_state: Dict[str, Any],
    ) -> Optional[Dict[str, Any]]:
        target = self._copy_target(joker_name, context, game_state)
        if target is None:
            return None

        target_index, target_joker = target
        target_name = self._joker_name(target_joker)
        if not target_name:
            return None

        copied_context = dict(context)
        copied_context["joker_index"] = target_index
        copied_context["copy_joker_indexes"] = tuple(
            list(context.get("copy_joker_indexes", ())) + [context.get("joker_index")]
        )
        copied_context["copy_depth"] = int(context.get("copy_depth", 0)) + 1
        return self.apply_joker_effect(target_joker, copied_context, game_state)

    def _copied_joker_retrigger_count(
        self,
        joker_name: str,
        context: Dict[str, Any],
        game_state: Dict[str, Any],
        ...
        copied_context["copy_depth"] = int(context.get("copy_depth", 0)) + 1
        return self.get_retrigger_count(target_joker, copied_context, game_state)
    ...
    def _copy_target(
        ...
        depth = int(context.get("copy_depth", 0))
        visited_indexes = {idx for idx in context.get("copy_joker_indexes", ()) if idx is not None}
        if depth >= self._COPY_GUARD_LIMIT or joker_index in visited_indexes:
            return None

        jokers = game_state.get("jokers", [])
        if joker_name == "Blueprint":
            target_index = joker_index + 1
        else:
            if joker_index == 0:
                return None
            target_index = self._leftmost_joker_index(jokers)

        if target_index is None or target_index == joker_index:
            return None
        if target_index in visited_indexes:
            return None
        if not 0 <= target_index < len(jokers):
            return None

        target_joker = jokers[target_index]
        if self._joker_disabled(target_joker):
            return None
        return target_index, target_joker

    def _leftmost_joker_index(self, jokers: List[Any]) -> Optional[int]:
        for joker_index, joker in enumerate(jokers):
            if self._joker_name(joker):
                return joker_index
        return None
    ...
    def _scoring_effects(
        ...
        if joker_name == "Hit the Road":
            x_mult = self._state(joker_name, x_mult=1.0)["x_mult"]
            return {"x_mult": x_mult} if x_mult > 1 else None
        if joker_name == "Madness":
            joker_index = context.get("joker_index")
            x_mult = self._game_state_joker_float(game_state, "madness_xmult", joker_index, default=1.0)
            return {"x_mult": x_mult} if x_mult > 1 else None
        if joker_name == "Obelisk":
            x_mult = self._state(joker_name, x_mult=1.0)["x_mult"]
            return {"x_mult": x_mult} if x_mult > 1 else None
    ...
    def end_of_round_effects(self, game_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for joker_index, joker_name in self._active_jokers(game_state):
            ...
            if joker_name == "Popcorn":
                ...
                effects.append({
                    "add_sell_value_to_joker_index": joker_index,
                    "sell_value_bonus": 3,
                })
            elif joker_name == "Gift Card":
                effects.append({
    ...
    def _game_state_joker_float(
        ...
        *,
        ...
        values = game_state.get(key, {})
        if not isinstance(values, dict) or joker_index is None:
            return default
        for candidate in (joker_index, str(joker_index)):
