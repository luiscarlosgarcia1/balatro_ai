...
class ShopPhaseHandler:
    ...
    def _handle_end_shop(self) -> Tuple[float, bool, Dict]:
        ...
        perkeo_created: List[str] = []
        for joker in self.state.jokers:
            if joker.name != "Perkeo" or not self.state.consumables:
                continue
            source_idx = self.rng.choice("joker_effects", list(range(len(self.state.consumables))))
            duplicated = self.state.consumables[source_idx]
            if self.state.add_consumable(duplicated, negative=True):
                perkeo_created.append(duplicated)
        ...
        if perkeo_created:
            info['perkeo_created'] = perkeo_created

        return -0.05, False, info  # time pressure: each shop visit must pay off
    
    def _handle_reroll(self) -> Tuple[float, bool, Dict]:
        """Handle rerolling the shop."""
        if not self.state.can_afford_shop_cost(self.state.shop_reroll_cost):
    ...
    def _handle_buy_item(self, action: int) -> Tuple[float, bool, Dict]:
        ...
        # Check affordability
        if not self.state.can_afford_shop_cost(item.cost):
    ...
    def _handle_sell_joker(self, action: int) -> Tuple[float, bool, Dict]:
        ...
        expected_sell_value = self.state.get_joker_sell_value(joker_idx)
        try:
        ...
        self._sync_player_state()
        if expected_sell_value != sell_value:
            self.state.money += expected_sell_value - sell_value
            sell_value = expected_sell_value
            if self.shop and self.shop.player:
                self.shop.player.chips = self.state.money
        if self.shop is not None:
            if hasattr(self.shop, "_refresh_passive_shop_state"):
                self.shop._refresh_passive_shop_state(skip_increment=True)
            if hasattr(self.shop, "_refresh_inventory_costs"):
                self.shop._refresh_inventory_costs()
            self.state.shop_reroll_cost = int(self.shop.reroll_cost * self.shop._cost_mult())
    ...
    def _handle_sell_consumable(self, action: int) -> Tuple[float, bool, Dict]:
        ...
        sell_value = self.state.get_consumable_sell_value(consumable_idx)
        consumable_name = self.state.remove_consumable(consumable_idx)
    ...
    def _create_player_state(self) -> PlayerState:
        ...
        player = PlayerState(
            ...
            debt_floor=self.state.get_debt_floor(),
    ...
    def _sync_player_state(self):
        ...
        if self.shop and self.shop.player:
            ...
            self.shop.player.debt_floor = self.state.get_debt_floor()
    ...
    def _sync_inventory_from_player(self, shop_info: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.state.negative_consumables = [
            idx for idx in self.state.negative_consumables if 0 <= idx < len(self.state.consumables)
        ]
    ...
    def _calculate_consumable_sell_value(self, consumable_name: str) -> int:
        """Return the current sell value for the first matching owned consumable."""
        for consumable_idx, owned_name in enumerate(self.state.consumables):
            if owned_name == consumable_name:
                return self.state.get_consumable_sell_value(consumable_idx)
        return 0
