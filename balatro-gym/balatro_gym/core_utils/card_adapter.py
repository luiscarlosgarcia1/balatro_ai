"""Canonical card adapters shared by scoring and consumable flows.

These shapes are intentionally small, but they are now part of the verified
adapter boundary because both scoring and consumable application depend on
their field layout.
"""
...
class CardAdapter:
    """Convert between the core card model and subsystem-specific shapes."""
    ...
    def _card_state(
        ...
        state: Optional[UnifiedGameState],
    ) -> Optional[CardState]:
        if state is None or card_idx is None:
            return None
        return state.card_states.get(card_idx) or CardState(card_idx)
    ...
    def to_scoring_format(
        ...
        Balatro's scoring pipeline distinguishes the card's printed base value
        from enhancement/edition side effects. This adapter preserves that split
        by exposing the unmodified base chip value while surfacing modifier
        state separately.
        """
        base_chips = card.rank.base_chips
        card_state = cls._card_state(card_idx, state)
