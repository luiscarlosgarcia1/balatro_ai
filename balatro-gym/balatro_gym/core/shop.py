...
class Shop:
    ...
    def _refresh_passive_shop_state(self, *, skip_increment: bool) -> None:
        chaos_count = self._count_joker(CHAOS_THE_CLOWN_ID)
        self.free_rerolls_remaining = max(0, chaos_count - self.free_rerolls_used)
        self._update_reroll_cost(skip_increment=skip_increment)
    ...
    def step(self, action_id: int):
        ...
        if self.player.chips - int(getattr(self.player, "debt_floor", 0) or 0) < item.cost:
        ...
        if verb == "buy_pack":
            ...
            self._refresh_passive_shop_state(skip_increment=True)
