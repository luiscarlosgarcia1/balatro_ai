...
class BossBlindManager:
    ...
    def _iter_active_jokers(game_state: Dict[str, Any]):
        """Yield active non-disabled joker names from a state dictionary."""
        for joker in list(game_state.get("jokers", []) or []):
            if isinstance(joker, dict):
            ...
            if disabled or not joker_name:
                continue
            yield str(joker_name)

    @classmethod
    def _normal_probability(cls, game_state: Dict[str, Any]) -> float:
        """Return the run's normal probability multiplier for boss chance effects."""
        multiplier = 1.0
        for joker_name in cls._iter_active_jokers(game_state):
            if joker_name == "Oops! All 6s":
                multiplier *= 2.0
        return multiplier

    @classmethod
    def boss_disabled_by_jokers(cls, game_state: Dict[str, Any]) -> bool:
        """Return whether an active real Chicot should disable the current boss."""
        return any(joker_name == "Chicot" for joker_name in cls._iter_active_jokers(game_state))
    ...
    def disable_boss_blind(
        ...
        *,
        ...
        if hasattr(state, "sync_passive_joker_effects"):
