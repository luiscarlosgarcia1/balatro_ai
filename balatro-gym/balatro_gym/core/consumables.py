...
def _active_consumable_count(game_state: Dict[str, Any]) -> int:
    consumables = list(game_state.get('consumables', []) or [])
    negative_indexes = {
        int(idx)
        for idx in game_state.get('negative_consumable_indexes', []) or []
...
def _can_add_consumable(game_state: Dict[str, Any], *, negative: bool = False) -> bool:
    if negative:
        return True
    return _active_consumable_count(game_state) < game_state.get('consumable_slots', 2)
...
class TarotEffects:
    ...
    def apply_tarot(tarot: TarotCard, game_state: Dict, 
        ...
        if tarot == TarotCard.THE_FOOL:
            ...
            has_capacity = _can_add_consumable(game_state)
            ...
            for _ in range(2):
                ...
                if _can_add_consumable(game_state):
